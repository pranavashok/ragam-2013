...
if (isset($_POST["signup"])) {
  ...
  if ($user == $admin_uname && $pass == $admin_pass) {
    ...
    header("Location: $ad_page");
    _exit();
  } else if ($user == $mailer_uname && $pass == $mailer_pass) {
    $_SESSION['type'] = 'ML';
    header("Location: $ml_page");
    _exit();
  } else if ($user == $college_uname && $pass == $college_pass) {
    $_SESSION['type'] = 'CL';
    header("Location: cl.php");
    _exit();
  } else if ($user == $publicity_uname && $pass == $publicity_pass) {
    $_SESSION['type'] = 'PL';
    header("Location: pl.php");
    _exit();
  }
...
<head>
  ...
  <title>Ragam 13 on Tathva 12 CMS: Start here!</title>
